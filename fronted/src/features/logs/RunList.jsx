...
function RunRow({ index, style, runs, selectedRunId, onSelect, pipelineProjects }) {
  ...
  return (
    <li
      ...
      className={`relative pl-3 pr-3 py-2.5 cursor-pointer transition-colors border-b border-gray-200 dark:border-gray-800 ${
        isActive
...
export default function RunList({ runs, selectedRunId, onSelect, loading, pipelineProjects = {} }) {
  ...
  return (
    <List
      ...
      rowHeight={(index) => {
        const run = runs[index]
        const hasProject = run.pipeline_id && pipelineProjects[run.pipeline_id]
        return hasProject ? 105 : 95
      }}
      rowComponent={RunRow}
      rowProps={{ runs, selectedRunId, onSelect, pipelineProjects }}
      style={{ width: '100%' }}
    />
  )
}
